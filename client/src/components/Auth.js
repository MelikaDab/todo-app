...
const Auth = () => {
  ...
  const [isLogIn, setIsLogIn] = useState(true) // if we are just logging in or if we are signing up
  ...
  const handleSubmit = async (e, endpoint) => {
    ...
    }) // construct a url using endpoint
    ...
    if (data.detail) {
      ...
      setCookies('Email', data.email)     // review what cookies are
